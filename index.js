...
// Function hiển thị thông tin IP server + địa chỉ truy cập
async function getIPserver() {
    try {
        ...
        logger(`http://localhost:${PORT}`, `| Địa chỉ truy cập |`);
...
// Hàm logger
function logger(value, label) {
    console.log(`${gradient.pastel(label)} ${value}`);
}

// Serve file index.html từ thư mục hiện tại
