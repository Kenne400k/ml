...
function scheduleRestart(timeString, api) {
    ...
    let next = moment().tz('Asia/Ho_Chi_Minh').hour(hour).minute(minute).second(second);

    if (now.isAfter(next)) {
        next.add(1, 'day'); // Nếu giờ đó đã qua thì hẹn vào ngày mai
    }

    const delay = next.diff(now);
    console.log(`[AUTO-RESTART] Đã lên lịch restart lúc ${next.format('YYYY-MM-DD HH:mm:ss')}`);

    setTimeout(() => {
        const logTime = moment().tz('Asia/Ho_Chi_Minh').format('YYYY-MM-DD HH:mm:ss');
        const logMessage = `[${logTime}] Restart bot theo lịch đã định...\n`;
        fs.appendFileSync('restart.log', logMessage);
        console.log(logMessage);

        // Gửi thông báo đến các nhóm (thread)
        api.getThreadList(100, null, ['INBOX']).then(list => {
            ...
            setTimeout(() => {
                exec('node index.js', (err, stdout, stderr) => {
                    ...
                    if (stderr) {
                        fs.appendFileSync('restart.log', `[${logTime}] Lỗi stderr: ${stderr}\n`);
                        console.error('[AUTO-RESTART]', stderr);
                        return;
                    }
                    console.log(`[AUTO-RESTART] Output: ${stdout}`);
                });

                process.exit(1); // Dừng tiến trình hiện tại sau khi restart
            }, 2000);
        });
...
//========= Auto Restart Example Usage =========//
module.exports.onLoad = function ({ api }) {
    // Đặt lịch cho các khung giờ cần restart
    scheduleRestart('12:00:00', api);  // 12h trưa
    scheduleRestart('01:50:00', api);  // 1h31 sáng (có thể điều chỉnh lại giờ theo nhu cầu của bạn)
};
/////////////////////////////////////////
//========= Load language use =========//
/////////////////////////////////////////
